...
/// A collection of [TextFragment]s extracted from PDF pages.
///
/// Uses **1-based indexing** to match the Aspose.PDF API convention:
/// `get(1)` returns the first fragment.
///
public class TextFragmentCollection implements Iterable<TextFragment> {
    ...
    /// Returns the number of fragments (alias for [#getCount()]).
    ///
    /// @return the fragment count
    public int size() {
        return fragments.size();
    ...
    /// @return `true` if the fragment was found and removed

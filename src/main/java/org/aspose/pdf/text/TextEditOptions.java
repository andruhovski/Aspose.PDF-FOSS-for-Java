...
/// underline detection) are performed by [TextFragmentAbsorber],
/// [TextSegment] and the content-editing facades.
///
/// API-compatible with `Aspose.Pdf.Text.TextEditOptions`. See
/// [the
/// Aspose reference](https://reference.aspose.com/pdf/net/aspose.pdf.text/texteditoptions/).
public class TextEditOptions {
    ...
    public enum NoCharacterAction {
        ...
        ReplaceFonts,
        /// Replaces the font with the user-defined [`replacement font`][#getReplacementFont()].
    ...
    /// [FontReplace#Default], [NoCharacterAction#ThrowException]).
    public TextEditOptions() {
    }

    /// Initializes new options for the specified language transformation permission.
    ///
    /// @param allowLanguageTransformation`true` to allow language transformation
    ...
    /// @return `true` if language transformation is allowed (default `true`)
    ...
    /// @return the fallback font used when [NoCharacterAction#UseCustomReplacementFont] is active
    ...
    ///
    /// @return `true` to attempt reading underline state from source content
    public boolean isToAttemptGetUnderlineFromSource() {
        return toAttemptGetUnderlineFromSource;
    }

    /// @param toAttemptGetUnderlineFromSource`true` to attempt reading underline state from source content

...
public class TextState {
    ...
    /// Full [Font] object as set via [#setFont(Font)]; carries the
    /// embedded flag, raw bytes and disk path. Layout consults this — not just
    /// the legacy [#fontName] string — when deciding whether to inline a
    ...
    /// Mirrors the Aspose.PDF `new TextState(double size)` constructor.
    ///
    ...
    /// `new TextState(foreground, background, fontStyle, font, size)`
    ...
    /// @param fontStyle  the font style bitmask (see [FontStyles])
    ...
    /// Returns the font as a [Font] object.
    ...
    /// Sets the font from a [Font] object. The full object is retained
    ...
    /// Returns the font style bitmask (see [FontStyles]).
    ///
    ...
    /// Sets the font style bitmask (see [FontStyles]).
    ///
    ...
    ///
    /// When the underline state transitions from `true` to `false` on a
    /// fragment whose underline was drawn as explicit path operators in the source
    /// content (detected via `TextEditOptions.ToAttemptGetUnderlineFromSource`),
    /// any registered [removal hooks][#addUnderlineRemovalHook] are invoked
    ...
    /// `setUnderline(false)` edit can strip them on save.
    ///
    /// @param hook the removal action (ignored if `null`)

...
/// Represents a single cell within a [Row] of a [Table].
///
/// A cell contains a collection of [BaseParagraph] elements (via [Paragraphs])
...
public class Cell {
    ...
    /// [FormFieldParagraph] adapter and appends it to this cell's
    /// paragraphs. Mirrors Aspose .NET's `cell.Paragraphs.Add(field)`
    /// usage for fields that don't extend [BaseParagraph].
    ///
    ...
    /// Convenience overload that wraps a [org.aspose.pdf.forms.RadioButtonOptionField]
    /// in a [FormFieldParagraph] adapter and appends it to this cell's
    /// paragraphs. Mirrors Aspose .NET's `cell.Paragraphs.Add(option)`
    ...
    /// Appends a [BaseParagraph] (text, image, table, etc.) directly.
    /// Equivalent to `getParagraphs().add(paragraph)` but provided so
    /// callers don't have to switch between `add(...)` and
    /// `getParagraphs().add(...)` depending on the argument type.
    ///
    ...
    ///
    /// @return the margin info representing cell padding, or `null`
    ...
    ///
    /// @return `true` if no border should be drawn
    ...
    ///
    /// @param noBorder`true` to suppress border drawing
    ...
    ///
    /// @return `true` if word wrapping is enabled; defaults to `true`
    ...
    ///
    /// @param wordWrapped`true` to enable word wrapping

...
/// [java.util.regex.Pattern]. The ES3 pattern is translated to Java regex
/// syntax (strategy (b); see `docs/xfa-dev/REGEX_STRATEGY.md`). The `global`,
/// `ignoreCase` and `multiline` flags and the mutable
/// `lastIndex` are exposed as own properties.
public final class JSRegExp extends JSObject {
    ...
    /// @return the current `lastIndex` as an int.

...
/// [java.util.regex.Pattern] (strategy (b) of `docs/xfa-dev/REGEX_STRATEGY.md`).
///
/// Most ES3 syntax maps directly onto `java.util.regex`. The one rewrite
/// applied is the non-multiline `^`/`$` anchor correction (see
/// [#translateAnchors]) — Java's `$` otherwise matches before a
/// trailing line terminator, which ECMAScript (without `m`) does not.
/// Remaining minor nuances (`m`-flag line-terminator set) are documented and
...
final class RegexTranslator {
    ...
    /// @param flags  flag characters (`g`, `i`, `m`)
    ...
    /// Corrects the one ECMAScript-vs-`java.util.regex` anchor divergence that bites: without
    /// the `m` flag, ECMAScript `^`/`$` match only the start/end of the whole input,
    /// whereas Java's `$` also matches _before a trailing line terminator_ (so
    /// `/^\\d+$/.test('123\\n')` is `false` in ECMAScript but would be `true` on raw
    /// Java). We map the top-level (outside a character class, unescaped) `^`→`\\A` and
    /// `$`→`\\z` (absolute input boundaries). With `m`, Java's `MULTILINE` line
    /// semantics are kept (the close approximation), so the anchors are left untouched.
    ///
    /// @param source   the ES regex body
    /// @param multiline whether the `m` flag is set
    ...
    /// Validates the flag string (only `g`/`i`/`m`, no repeats).
    ///
    /// @param flags flag string
    /// @return `true` if valid for ES3

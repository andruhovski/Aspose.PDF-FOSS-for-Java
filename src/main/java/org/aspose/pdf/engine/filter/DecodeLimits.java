 ...
 * stream into an unbounded {@code ByteArrayOutputStream} ends in
 * {@link OutOfMemoryError}, which poisons the whole JVM — every other thread
 ...
 * Override with the system property {@code aspose.pdf.maxDecodedStreamBytes}
...
public final class DecodeLimits {
    ...
    /**
     * Throws when {@code decodedSoFar} exceeds the cap.
     *
     * @param decodedSoFar bytes produced by the filter so far
     * @param limit        the cap obtained from {@link #maxDecodedBytes()} once

...
/// the base class [PdfFont] already handles. For rendering, the
/// renderer executes the glyph's content stream with the CTM set to
/// `FontMatrix × textState × Tm × CTM` (see §9.6.5 and §9.4.4);
/// this class exposes [#getFontMatrix()], [#getCharProc(int)]
/// and [#getFontResources()] for that purpose.
///
public class Type3Font extends PdfFont {
    ...
    /// `wGlyph * FontMatrix.a` advance in `TextRenderer`.
    ///
    ...
    ///
    /// @return the font matrix; identity-scaled `[0.001 0 0 0.001 0 0]`

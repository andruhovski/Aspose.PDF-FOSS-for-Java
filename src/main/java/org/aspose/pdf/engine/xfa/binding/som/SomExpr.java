...
///
/// An expression is an optional accessor [Root] followed by a sequence of
/// [Step]s: dotted name paths (`Receipt.Total_Price`), index notation
/// (`name[n]`, `name[*]`, `name[predicate]`), class syntax
/// (`#subform[1]`, `#dataValue`), property syntax (`.#name`),
/// and parent navigation (`..`).
///
/// This is a pure structural language — predicates are limited to structural /
/// comparison expressions; script-bearing predicates are flagged
/// ([Predicate#script]) and not evaluated here (Stage B). The resolver does
/// NOT depend on the JavaScript engine.
public final class SomExpr {
    ...
    public enum Root {
        ...
        NONE,
        /// `$xfa` — the application/data model root.
        XFA,
        /// `$template` — the template root.
        TEMPLATE,
        /// `$data` — the data model root.
        DATA,
        /// `$form` — the form (merged) root.
        FORM,
        /// `$record` — the current data record.
        RECORD,
        /// `$dataWindow` — the data window.
        DATAWINDOW,
        /// `$` — the current node.
        CURRENT
    }
    ...
    public static final class Index {
        ...
        public enum Kind {
            ...
            NONE,
            /// `[*]` — every sibling of that name.
            ALL,
            /// `[n]` — the n-th occurrence (0-based).
            NUM,
            /// `[predicate]` — filter candidates by a predicate.
            PRED
        }
    ...
    /// A structural/comparison predicate `relativePath OP literal` (or a bare path = truthiness).
    public static final class Predicate {
        ...
        /// `true` if the predicate contains script (FormCalc/JS) — deferred to Stage B, not evaluated.
    ...
    /// A named child step (`name`, possibly indexed).
    ...
    /// A class step (`#subform`, `#dataValue`, possibly indexed).
    ...
    /// A property step (`.#name`, `.#x`) — yields an attribute/property value.
    ...
    /// @return `true` if the last step is a property accessor.
    public boolean endsWithProperty() {
        return !steps.isEmpty() && steps.get(steps.size() - 1) instanceof PropertyStep;
    }

    /// @return `true` if any predicate in the expression contains script (deferred).

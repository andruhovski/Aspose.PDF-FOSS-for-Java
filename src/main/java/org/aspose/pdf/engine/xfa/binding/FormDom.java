...
///
/// Exposes the merged root, the enumerated fields (name → value / items /
/// UI / binding kind), and any SOM predicates that contained script and were
/// therefore deferred (not evaluated) per the A4 scope.
public final class FormDom {
    ...
    /// @return `true` if this was an empty (form-only, no data) merge.
    ...
    /// The fields bound on the `<pageSet>` master pages (page furniture: headers, footers,
    /// address blocks). These are a **separate channel** from [#getFields()] — they are
    ...
    /// The bound `<pageArea>` elements of the first `<pageSet>`, in declaration order.
    ...
    /// Records a variable-occurrence container that bound to ZERO instances (an `<occur min="0">`
    /// subform whose data was absent / that is shown only when a script `addInstance()`s it). The
    /// paired `[parentElement, templateElement]` lets the script host create an instanceManager
    ...
    /// @return the zero-instance variable-occurrence slots as `[parentElement, templateElement]`
    ///         pairs (see [#addZeroOccurSlot]); empty when every occur container bound ≥1 instance
    public List<Element[]> getZeroOccurSlots() {
        return zeroOccurSlots;
    }

    /// Registers a new field for a runtime-added node (Stage B instanceManager `addInstance`):
    /// builds a [FormField] for `formNode` and appends it so the render track sees the new
    /// instance's field and value. The binding kind is `UNBOUND` (created at runtime, not by a
    /// data merge).
    ///
    ...
    /// Removes a field (Stage B instanceManager `removeInstance`).
    ///

...
/// **initialize** events (seed) → **calculate** (derive values, in SOM-dependency topological
/// order with cycle detection) → **ready** events. `validate` scripts are evaluated
/// report-only (a false result flags the field, it does not block). Computed/seeded values are
/// written back through [org.aspose.pdf.engine.xfa.binding.FormField] so they flow into the
...
public final class XfaScripting {
    ...
    /// JavaScript-only shape markers. An untyped `<script>` (XFA spec default is FormCalc) is
    ...
    /// (the B3-DIAG "FormCalc-as-JS SyntaxError"). `.rawValue`/`==` are deliberately absent
    ...
    ///
    /// @param ct  the `contentType` attribute (may be null/empty)
    /// @param src the script source
    /// @return `true` for FormCalc, `false` for JavaScript
    ...
    /// Executes the load-time scripts of `dom` and writes computed values back into it.
    ///
    ...
    /// subform add/remove). initialize events run first (they may `addInstance`); calculate /
    /// validate / ready are then collected from the **mutated** tree so a new instance's own scripts
    /// participate, and computed values are written back into `dom`.
    ///
    /// @param dom the merged Form DOM (mutated in place)
    /// @param tpl the template (instanceManager occur limits + clone prototypes), or `null`
    ...
    /// Whether `ff` carries an imported data value (a real binding, not none/unbound).
    ...
    /// Whether `s` is a meaningful value — non-null, non-empty, and not numeric zero.
    private static boolean isNonDegenerate(String s) {
        return s != null && !s.isEmpty() && !isZeroNumeric(s);
    }

    /// Collects `initialize` (and, when `ready` non-null, ready) event scripts.
    ...
    /// Referenced field names from a calculate script (for dependency ordering): `resolveNode("…")`
    /// targets + `X.rawValue` for JS; additionally every bare SOM identifier for FormCalc (which
    /// references siblings by bare name, e.g. `numTotal + numStateTax`), excluding FormCalc
    ...
    /// Whether `s` is a number that parses to exactly zero ("0", "0.00", "0.0", …).

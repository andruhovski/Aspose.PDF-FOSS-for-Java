...
///
/// Numeric values are formatted using [Locale#US] to ensure decimal points
...
public class ContentStreamBuilder {
    ...
    /// Resource name (e.g. "F2") → the embedded `/Type0` font dictionary registered via
    /// [#registerEmbeddedFont]. The caller's font-merge step writes these dicts onto the page's
    /// `/Resources/Font` instead of a standard-14 `/Type1` dict.
    ...
    /// [#registerImage(String, org.aspose.pdf.engine.pdfobjects.PdfStream)]. The caller's
    /// resource-merge step attaches these onto the page's `/Resources/XObject`.
    ...
    /// (Type0 with a TrueType descendant). For these, [#showText] emits
    /// each character as a big-endian `<gid>` hex string instead of a
    /// WinAnsi byte. Mapped to the [org.aspose.pdf.engine.font.ttf.TrueTypeReader]
    /// that provides the Unicode→glyph\_id translation.
    private final Map<String, org.aspose.pdf.engine.font.ttf.TrueTypeReader> type0Readers
            = new LinkedHashMap<>();

    /// Currently active font resource name from the last [#setFont] call.
    ...
    /// Marks `resourceName` as a Type0/Identity-H font. Subsequent
    /// [#showText] calls — while this font is active — will emit
    /// 2-byte glyph IDs via the supplied `reader`'s cmap instead of
    /// single-byte WinAnsi codes. Call once per page after the font has been
    /// registered with [#registerFont].
    ///
    /// @param resourceName the `/Fn` name on /Resources/Font
    ...
    /// Emits `text` as a hex string of 2-byte glyph IDs for an
    /// Identity-H Type0 font. Each input _codepoint_ — supplementary-
    /// plane characters included — is translated through the font's cmap to
    /// a single 16-bit GID; missing glyphs fall back to `.notdef`
    /// (GID 0).
    ///
    /// One codepoint = one 2-byte CID, even when the Java string carries
    /// the codepoint as a UTF-16 surrogate pair. The decoder (`TextRenderer`)
    /// walks the resulting bytes in 2-byte strides, so this stays balanced.
    ...
    /// Emits the m (move-to) operator, beginning a new subpath at `(x,y)`.
    ///
    ...
    /// Emits the l (line-to) operator, appending a straight segment to `(x,y)`.
    ///
    /// @param x the x coordinate
    /// @param y the y coordinate
    ...
    /// Emits `W n` — intersects the clipping path with the current path and then ends the path
    /// without painting. Use after defining a path (e.g. [#rectangle]) to clip subsequent marks
    /// to that region (ISO 32000-1:2008, §8.5.4). Pair with [#saveState]/[#restoreState]
    /// to scope the clip.
    public void clip() {
        emit("W n\n");
    }
    ...
    /// Registers an embedded `/Type0` font and returns its resource name. The font is marked as
    /// Identity-H (subsequent [#showText] emits 2-byte GIDs via `reader`), and the supplied
    /// Type0 dictionary is recorded so the caller's font-merge step can attach it to the page
    /// `/Resources/Font`. Re-registering the same `fontKey` returns the existing name.
    ///
    /// @param fontKey   a stable unique key for this font+style (e.g. `"arial-b"`)
    /// @param type0Dict the assembled `/Type0` font dictionary
    /// @param reader    the TrueType reader providing Unicode→GID for encoding
    /// @return the resource name (e.g. `"F2"`)
    ...
    /// @return resource name → embedded `/Type0` font dictionary, for the font-merge step
    ...
    /// the caller's resource-merge step can attach it to the page `/Resources/XObject`. Paint the
    /// image with `q  w 0 0 h x y cm  /name Do  Q` (the `cm` scales the 1×1 image space to
    /// the target rectangle). Re-registering the same `key` returns the existing name.
    ///

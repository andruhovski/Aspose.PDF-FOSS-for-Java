...
/// operator, pass it to [OperatorCollection#accept(IOperatorSelector)],
/// then read the matches via [#getSelected()]. An operator matches when it
/// is an instance of the prototype's runtime class. Because Aspose operator
/// types form a flat sibling hierarchy (e.g. `ShowText`,
/// `SetGlyphsPositionShowText`, … all extend `TextShowOperator`
/// directly), this is equivalent to an exact-type match for the operator types
/// the API exposes.
///
/// Mirrors Aspose.PDF for .NET `OperatorSelector` (ISO 32000-1:2008, §8.2).
///
public class OperatorSelector implements IOperatorSelector {
    ...
    /// Visitor callback invoked by [OperatorCollection#accept]. Adds the
    ...
    /// `Selected` property; callers may iterate or `addAll` from it.
    ///

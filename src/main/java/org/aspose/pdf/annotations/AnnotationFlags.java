...
///
/// Each enum constant carries the actual bit value (1 << (n − 1)) for the
/// spec's 1-indexed bit position, so [#getBit()] can be used directly
/// for bitwise composition.
///
/// Use [#fromBits(int)] to decode a raw `/F` entry into a typed
/// [EnumSet] and [#toBits(EnumSet)] to re-encode.
public enum AnnotationFlags {
    ...
    /// Decodes a raw `/F` bitfield into a typed [EnumSet].
    ///
    /// @param bits the raw flag bits
    /// @return a set with every flag whose bit is set in `bits`
    ...
    /// Encodes an [EnumSet] of flags into a raw bitfield suitable for
    /// `/F`.
    ///
    /// @param flags the set of flags (must not be null)
    /// @return the OR of every member's [#getBit()]

...
public class WatermarkAnnotation extends MarkupAnnotation {
    ...
    /// Returns the watermark text (alias for [#getContents()]).
    /// Watermark text content is stored in the standard `/Contents` entry.
    ///
    ...
    /// Sets the watermark text (alias for [#setContents(String)]).
    /// Stored in the standard `/Contents` entry.
    ///
    ...
    /// Returns the watermark opacity (`/CA`, 0.0 = fully transparent,
    ...
    /// Sets the watermark opacity (`/CA`). Values outside [0, 1] are not
    ...
    /// Returns the watermark rotation angle in degrees (`/Rotate`).

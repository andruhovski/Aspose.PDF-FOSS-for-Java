...
/// Centralised logging configuration for the `org.aspose.pdf` library.
///
/// By default the library is **silent** — its loggers are
/// configured to [Level#OFF], so no output is emitted to host
...
/// </pre>
///
/// This class never touches the JUL root logger — only the
/// `org.aspose.pdf` subtree — so it will not interfere with the host
/// application's existing logging configuration.
public final class AsposePdfLogging {

    /// The library's root logger; all `org.aspose.pdf.*` classes log through it.
    ...
    /// Re-reads [#LOG\_PROPERTY] and configures the library logger.
    ...
    /// Programmatically sets the library log level. A `null` level silences
    /// the library ([Level#OFF]).
    ///
    /// @param level the level to apply, or `null` for [Level#OFF]
    public static synchronized void setLevel(Level level) {
        applyLevel(level != null ? level : Level.OFF);
    }

    /// Returns the effective level on the `org.aspose.pdf` logger.
    ///
    /// @return the current effective level (never `null`)

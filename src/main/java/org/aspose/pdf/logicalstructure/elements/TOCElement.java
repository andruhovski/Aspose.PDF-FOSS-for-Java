...
///
/// A TOC element may only be a direct child of the root structure element
/// (the /Document element). Its children must be TOCIElement instances.
///
/// Validation rules:
///
///   - TOC can only be a child of the root element
///   - TOC children must be TOCI elements only
///   - Multiple TOC elements at the root level are permitted
public class TOCElement extends Element {
    ...
    /// Appends a child element. Only [TOCIElement] children are allowed.
    ///
    ...
    /// a title set via `TocInfo.setTitle()`.
    ///
    ...
    /// Returns the linked TOC page, or `null` if not set.
    ///
    ...
    /// Returns the linked title header, or `null` if not set.
    ///

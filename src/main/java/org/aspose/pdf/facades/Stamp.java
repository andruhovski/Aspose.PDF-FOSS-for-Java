...
/// Represents a stamp in the facades API that can be applied via [PdfFileStamp].
///
/// A facade stamp wraps either a [FormattedText] (for text stamps) or an
/// image, along with positioning and rotation properties. Use [#bindLogo(FormattedText)]
/// to attach text content, and [#setOrigin(double, double)] to set the position.
///
public class Stamp {
    ...
    /// Creates a new empty `Stamp` instance.
    public Stamp() {
    }

    /// Binds a [FormattedText] as the text content (logo) of this stamp.
    ///
    ...
    /// [#bindImage(String)] — mirrors the Aspose .NET overload that
    /// accepts a file path under the same `BindLogo` name.
    ///
    ...
    /// Convenience overload: binds the first page of `pdfFile` as the
    /// stamp source.
    ///
    ...
    /// eagerly buffered into a [Document] so the caller can close it
    ...
    ///
    /// @return `true` if the stamp is a background element
    ...
    ///
    /// @param background`true` to render the stamp behind content
    ...
    /// Returns the bound [FormattedText], or `null` if none has been set.
    ///
    ...
    /// Returns the bound image file path, or `null`.
    ///
    ...
    /// Returns the bound image stream, or `null`.
    ///
    ...
    /// Returns the bound PDF file path, or `null`.
    ///
    ...
    /// Returns the bound PDF document, or `null`.
    ///

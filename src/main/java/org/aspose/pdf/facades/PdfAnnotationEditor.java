...
public class PdfAnnotationEditor {
    ...
    /// Creates a new `PdfAnnotationEditor` instance.
    public PdfAnnotationEditor() {
    }

    /// Creates a new `PdfAnnotationEditor` bound to the specified document.
    ///
    ...
    /// normal appearance stream (`/AP /N`) has that appearance burned into
    /// the page content stream — wrapped in a `q <CTM> cm ... Q` block
    /// whose CTM scales the appearance `/BBox` onto the annotation
    /// `/Rect` (ISO 32000-1:2008 §12.5.5) — and is then removed from the
    /// page `/Annots` array.
    ///
    /// Previously this only deleted annotations without burning their
    ...
    /// current author equals `oldAuthor`. Mirrors C#
    /// `PdfAnnotationEditor.ModifyAnnotationsAuthor(int, int, string, string)`.
    ...
    /// Imports annotations from an XFDF file (all types).
    /// This is an alias for [#importAnnotationFromXfdf(String)].
    ///
    /// @param filePath path to the XFDF file
    /// @throws IOException if reading fails
    ...
    /// Imports annotations from an XFDF input stream (all types).
    /// This is an alias for [#importAnnotationFromXfdf(InputStream)].
    ///
    /// @param stream XFDF input stream
    /// @throws IOException if reading fails
    ...
    /// This is an alias for [#importAnnotationFromXfdf(InputStream, AnnotationType\[\])].
    ///
    /// @param stream     XFDF input stream
    /// @param annotTypes annotation types to import (null = all)
    /// @throws IOException if reading fails

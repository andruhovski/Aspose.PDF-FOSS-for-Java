...
/// [FormEditor#addField(FieldType, String, String, int, double, double, double, double)].
/// Mirrors `Aspose.Pdf.Facades.FormFieldFacade`: collects font, colour and
...
public class FormFieldFacade {
    ...
    /// Returns the field background colour, or `null` when unset.
    ...
    /// Mirrors the C# property name `BackgroudColor` (note the typo in the
    /// original .NET API — Aspose preserves it for backward compatibility).
    /// Delegates to [#setBackgroundColor(Color)].
    public void setBackgroudColor(Color color) {
        setBackgroundColor(color);
    }

    /// Returns the field border colour, or `null` when unset.
    ...
    /// Returns the border style — one of `BorderStyle*` constants.
    public int getBorderStyle() {
        return borderStyle;
    }

    /// Sets the border style — one of `BorderStyle*` constants.

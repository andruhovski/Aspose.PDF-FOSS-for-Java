...
public class BackgroundArtifact extends Artifact {
    ...
    ///
    /// @return the background image stream, or `null` if not set
    ...
    /// Emits a `/Artifact BMC q <color> rg <pageRect> re f Q EMC`
    /// sequence covering the page's media box. Returns `null` when no
    /// background colour has been set (the parent [Page] then falls back

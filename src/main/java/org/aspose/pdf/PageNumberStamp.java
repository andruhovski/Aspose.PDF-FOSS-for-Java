...
///
/// Extends [TextStamp] and uses a format string where `#` is replaced
/// with the current page number and `$P` is replaced with the total number of pages.
/// For example, the default format `"Page # of $P"` renders as "Page 1 of 10".
///
public class PageNumberStamp extends TextStamp {
    ...
    ///
    /// In the format string, `#` is replaced with the current page number and
    /// `$P` is replaced with the total page count.
    ///
    ...
    ///
    /// `#` is replaced with the current page number and `$P` with the total pages.
    ///
    ...
    ///
    /// Replaces `#` with `(pageIndex + startingNumber)` and `$P`
    /// with `totalPages`.
    ///

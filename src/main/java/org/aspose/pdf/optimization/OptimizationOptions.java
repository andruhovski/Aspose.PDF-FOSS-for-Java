...
/// Options controlling [org.aspose.pdf.Document#optimizeResources(OptimizationOptions)].
///
/// API-compatible with Aspose.PDF's `Aspose.Pdf.Optimization.OptimizationOptions`
/// (also surfaced in the original API as the nested `Document.OptimizationOptions`).
/// Each flag toggles one optimisation pass. The structural passes
/// ([#setRemoveUnusedObjects], [#setRemoveUnusedStreams],
/// [#setLinkDuplicateStreams], [#setAllowReusePageContent]) are
/// implemented; the image- and font-rewriting passes are accepted for API
/// compatibility and applied on a best-effort basis.
public class OptimizationOptions {
    ...
    /// @return the JPEG quality (1-100) used when [#isCompressImages()] is set
    ...
    /// @return the maximum image resolution (DPI) used when [#isResizeImages()] is set
    ...
    /// `OptimizationOptions.CompressObjects`; the compact rewrite already
